...
Une situation ne porte aucun lien vers les notions. Le lien est porté par l'entité `Angle` (section 1.4), qui en est l'unique source de vérité.
...
Un angle est **une lecture pédagogique d'une situation à travers une notion**. Une même situation vue sous deux angles pose deux questions différentes et appelle deux réponses différentes.

C'est l'entité qui porte le lien entre `Situation` et `Notion` — elle en est l'unique source de vérité.

Fichier : `/data/angles/angles.json`

Exemple :

```json
{
  "id": "ANGLE_MAIRIE_01_LAICITE",
  "situation": "SIT_MAIRIE_01",
  "notion": "LAICITE_01",
  "role": "application",

  "question": "Le maire peut-il donner cet argent ?",
  "reponse_correcte": "Non",
  "reponse_incorrecte": "Oui",

  "explication_courte": "L'argent public ne finance pas la construction d'un lieu de culte.",
  "feedback_erreur": "Beaucoup pensent que la mairie peut aider une association religieuse comme les autres. En réalité, l'argent public ne construit pas de lieu de culte — mais il peut restaurer un monument classé."
}
```
...
|---|---|---|
| `explication_courte` | La question posée | La réponse est juste |
| `feedback_erreur` | L'erreur de raisonnement | La réponse est fausse |

`explication_courte` confirme le verdict, localement, sans enseigner la théorie générale. `feedback_erreur` traite la confusion elle-même — c'est le feedback élaboratif de la section 5.4.

**Règle L5 — Aucune anticipation de la boucle 2**

Ni `explication_courte` ni `feedback_erreur` ne doivent contenir un terme qui sera révélé en boucle 2 : le nom de la notion, un mot de son `lexique`, ou une formulation reprise de `definition_simple` ou `definition_officielle`.

Si la boucle 2 doit révéler le mot « laïcité », alors aucun angle ne peut l'écrire. Sinon la révélation a déjà eu lieu, et la boucle 2 perd son effet.

Cette règle est vérifiable automatiquement : les termes interdits se déduisent des champs de la notion.
...
**Mesure de richesse :** le nombre d'angles portés par une situation indique sa valeur pédagogique. Une situation qui n'en porte qu'un seul est peut-être trop pauvre pour mériter d'exister.
...
### 1.6 Entité `EtatNotion` (stockée dans le navigateur)
...
>
> ```
> Lis SPECIFICATION.md, section 1.6 et 1.7.
>
...
>   le critère d'acquisition de la section 1.7
...
Descriptif, vrai, non anxiogène. Le critère d'acquisition (§1.7) est directement l'interface.
...
| L5 | Aucun angle ne contient un terme révélé en boucle 2 (voir section 1.4) | Oui |
